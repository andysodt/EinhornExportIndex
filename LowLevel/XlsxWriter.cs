...
namespace NanoXLSX.LowLevel
{
    ...
    class XlsxWriter
    {
        ...
        {
            Styles.Font[] fontStyles = styles.GetFonts();
            StringBuilder sb = new StringBuilder();
            foreach (Styles.Font item in fontStyles)
            {
                sb.Append("<font>");
                if (item.Bold) { sb.Append("<b/>"); }
                if (item.Italic) { sb.Append("<i/>"); }
                if (item.Strike) { sb.Append("<strike/>"); }
                if (item.Underline != Styles.Font.UnderlineValue.none)
                {
                    if (item.Underline == Styles.Font.UnderlineValue.u_double) { sb.Append("<u val=\"double\"/>"); }
                    else if (item.Underline == Styles.Font.UnderlineValue.singleAccounting) { sb.Append("<u val=\"singleAccounting\"/>"); }
                    else if (item.Underline == Styles.Font.UnderlineValue.doubleAccounting) { sb.Append("<u val=\"doubleAccounting\"/>"); }
                    else { sb.Append("<u/>"); }
                }
                if (item.VerticalAlign == Styles.Font.VerticalAlignValue.subscript) { sb.Append("<vertAlign val=\"subscript\"/>"); }
                else if (item.VerticalAlign == Styles.Font.VerticalAlignValue.superscript) { sb.Append("<vertAlign val=\"superscript\"/>"); }
                ...
                if (item.Scheme != Styles.Font.SchemeValue.none)
                {
                    if (item.Scheme == Styles.Font.SchemeValue.major)
                    { sb.Append("<scheme val=\"major\"/>"); }
                    else if (item.Scheme == Styles.Font.SchemeValue.minor)
