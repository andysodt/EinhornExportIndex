...
namespace NanoXLSX.LowLevel
{
    ...
    public class StyleReader
    {
        ...
        private void GetFonts(XmlNode node)
        {
            ...
            {
                Styles.Font fontStyle = new Styles.Font();
                ...
                {
                    fontStyle.Underline = Styles.Font.UnderlineValue.u_single; // default
                    switch (attribute)
                    {
                        case "double":
                            fontStyle.Underline = Styles.Font.UnderlineValue.u_double;
                            break;
                        case "singleAccounting":
                            fontStyle.Underline = Styles.Font.UnderlineValue.singleAccounting;
                            break;
                        case "doubleAccounting":
                            fontStyle.Underline = Styles.Font.UnderlineValue.doubleAccounting;
                            break;
                    }
                }
                if (ReaderUtils.GetAttributeOfChild(font, "vertAlign", "val", out attribute))
                {
                    Styles.Font.VerticalAlignValue vertAlignValue;
                    if (Enum.TryParse<Styles.Font.VerticalAlignValue>(attribute, out vertAlignValue))
                    {
                ...
                if (ReaderUtils.GetAttributeOfChild(font, "scheme", "val", out attribute))
                {
                    ...
                    {
                        case "major":
                            fontStyle.Scheme = Styles.Font.SchemeValue.major;
                            break;
                        case "minor":
                            fontStyle.Scheme = Styles.Font.SchemeValue.minor;
                            break;
                    }
                }
        ...
        private void GetCellXfs(XmlNode node)
        {
            foreach (XmlNode childNode in node.ChildNodes)
            {
                if (ReaderUtils.IsNode(childNode, "xf"))
                {
                    ...
                    Styles.Font font = StyleReaderContainer.GetFont(id);
                    if (!hasId || font == null)
                    {
